...
int main(){
    ...
    cout << "IV: \n";
    for(int i = 0; i < 8;i++){
        ((unsigned char*)(&IV))[i] = 'a'+rand()%26;
        ((unsigned char*)(&IV2))[i] = ((unsigned char*)(&IV))[i];
        cout << ((unsigned char*)(&IV))[i];
    }cout << endl;

    // to sychronize with other alg's rands
    for(int i = 0; i < 8;i++)
        int ktmpA = 'a'+rand()%8;
